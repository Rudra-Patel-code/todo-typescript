...
function App() {
  ...
  const changeTabs = (tab: "all" | "isCompleted" | "isNotCompleted") => {
  ...
  useEffect(() => {
    switch (selectedOption) {
      case "isCompleted":
        setTodos(sourceTodo.filter((todo) => todo.isComplete === true));
        break;

      case "isNotCompleted":
        setTodos(sourceTodo.filter((todo) => todo.isComplete === false));
        break;
    ...
    <>
      ...
      {todos && todos.length > 0 ? (
        <div className={classNames("flex flex-col items-center p-3 ")}>
          ...
          />
          <div className=" w-full max-w-[800px] flex flex-col overflow-auto h-[calc(100vh-300px)] ">
            {todos?.map((todo) => (
        ...
        <div className={classNames("flex flex-col items-center p-3 ")}>
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-4 text-white">
            ...
            <h1 className="text-2xl font-extrabold md:text-4xl">
              No {selectedOption === "isCompleted" ? "completed" : "ending"}{" "}
              todos found?
            </h1>
          </div>
        </div>
